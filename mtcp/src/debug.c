...
DumpPacket(mtcp_manager_t mtcp, char *buf, int len, char *step, int ifindex)
{
    ...
    if (checksum(iph, iph->ihl)) { // DEFINE in debug.h by Tong SHEN!
		__sum16 org_csum, correct_csum;
		
		org_csum = iph->check;
		iph->check = 0;
		//correct_csum = ip_fast_csum(iph, iph->ihl);
        correct_csum = checksum(iph, iph->ihl); // DEFINE in debug.h by Tong SHEN!
...
unsigned short
checksum(const void *buf, unsigned int nword)
{
    unsigned short *bf = (unsigned short *)buf;
    unsigned long sum;
