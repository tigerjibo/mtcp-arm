...
InitializeServerThread(int core)
{
	...
	ctx->ep = mtcp_epoll_create(ctx->mctx, MAX_EVENTS);//建立epoll套接字fd
	if (ctx->ep < 0) {
...
CreateListeningSocket(struct thread_context *ctx)
{
	...
	listener = mtcp_socket(ctx->mctx, AF_INET, SOCK_STREAM, 0);//建立一个epoll套接字并返回它的id，即fd
	if (listener < 0) {
	...
	mtcp_epoll_ctl(ctx->mctx, ctx->ep, MTCP_EPOLL_CTL_ADD, listener, &ev);//为listener对应的套接字添加一个MTCP_EPOLLIN事件
...
RunServerThread(void *arg)
{
	...
	while (!done[core]) {
		nevents = mtcp_epoll_wait(mctx, ep, events, MAX_EVENTS, -1);//events记录发生的事件
		if (nevents < 0) {
			if (errno != EINTR)
